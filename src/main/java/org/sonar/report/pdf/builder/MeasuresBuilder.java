...
public class MeasuresBuilder {
    ...
    public List<String> getAllMetricKeysByProjectKey(String projectKey) throws HttpException, IOException {
        MetricQuery query = MetricQuery.byKey(projectKey);
        LOG.info("firing the request using the url : "+query.getUrl());
        List<Metric> allMetricKeysNodes = sonar.findAll(query);
        List<String> allMetricKeys = new ArrayList<String>();
        Iterator<Metric> it = allMetricKeysNodes.iterator();
        while (it.hasNext()) {
    ...
    public Measures initMeasuresByProjectKey(final String projectKey)
        ...
        if (measuresKeys == null) {
            LOG.info("trying to get all the metrices for projectkey...."+projectKey);
            //measuresKeys = getAllMetricKeys();
            measuresKeys = getAllMetricKeysByProjectKey(projectKey);
            LOG.info("got all the metrices....");
        }
